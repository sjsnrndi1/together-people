...
<head>
<meta charset="UTF-8">
<title>${userInfo.user_name} | 참여한 모임</title>
...
<body>
	...
	<form action = "mypage_joinBoard_delete" name = "mypageJoinBoardDelete" method = "POST" onsubmit = "return check()">
	<div class = "mypage_community_frame">
		...
		<div class = "mypage_community_content_frame">
			<div class = "mypage_community_content">
				<c:forEach items = "${joinBoardList }" var = "joinBoard">
					<c:if test = "${joinBoard.joinBoardUserId eq ssVar}">
						<table>
							<tr>
								<td style = "width : 2%;"><input type = "checkbox" name = "delete_board" value = "${joinBoard.joinBoardSubject}-${joinBoard.joinBoardNumber}"/></td>
								<td style = "width : 10%;">${joinBoard.joinBoardSubject}</td>
								<td style = "width : 40%;">
								<a href = "mypageContentView?boardNumber=${joinBoard.joinBoardNumber}&boardSubject=${joinBoard.joinBoardSubject}">${joinBoard.joinBoardTitle}</a>
								</td>
								<fmt:formatDate value = "${joinBoard.joinBoardDate}" pattern = "yyyy-MM-dd" var = "joinBoardDate"/>
								<td style = "width : 10%;">${joinBoardDate}</td>
								<td style = "width : 10%;">${joinBoard.joinBoard_joinUserNumber}명</td>
							</tr>
						</table>
					</c:if>
				</c:forEach>
